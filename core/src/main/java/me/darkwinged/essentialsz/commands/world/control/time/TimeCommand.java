            ...
            if (plugin.getConfig().getBoolean("Commands.Time", true)) {
                if (!(sender instanceof Player)) {
                    if (args.length < 2) {
                        ...
                        sender.sendMessage(ErrorManager.getErrors(Errors.SpecifyTime));
                        return true;
                    }
                ...
                if (player.hasPermission(Permissions.WorldTime) || player.hasPermission(Permissions.GlobalOverwrite)) {
                    if (args.length == 2) {
                        World world = Bukkit.getWorld(args[0]);
                        if (world == null) {
